...
- `configURL` / `clientId` are the only two things the RP needs to know about the IdP —
...
- `nonce` is generated fresh per attempt and round-trips through the minted JWT
  (`internal/jwtutil`), for replay binding. Note it goes inside `params`, not as a top-level field —
  newer Chrome/Edge deprecated the top-level form.
- This call is what triggers the entire browser-mediated sequence described in
...
manual call on button click — if those ever overlap, the second one throws
`NotAllowedError: Only one navigator.credentials.get request may be outstanding at one time`.
`requestCredential()` handles this with an `AbortController`: starting a new request aborts
whichever one was already running, instead of leaving both racing.
...
`mediation: "silent"` asks the browser to sign the user back in **with no UI at all** — useful for
"you were signed in last time, so just resume" without a jarring chooser popup on every visit. It
only succeeds if:
...
- the browser isn't in the post-sign-in **quiet period** (an anti-abuse cooldown — see §1) —
  which is why silent auto-reauth can appear to work sometimes and not others right after you've
  just signed in or disconnected; that's the browser throttling it, not a bug in this code.

When any of those conditions fail, the promise rejects and we just leave the "Sign in" button
visible — there's no special handling needed beyond catching the rejection, which is also why the
UI shows a transient "Checking for an existing sign-in…" status that clears itself rather than
displaying an alarming error for what's actually the expected, common case.
...
`.token` is whatever the IdP's assertion endpoint returned — in this demo, a JWT. The RP's own JS
...
the RP's responsibility.

## 3.5 What the RP deliberately does *not* need

- No CORS setup of its own — `/session` and `/disconnected` are same-origin fetches from the
  page's own JS to its own backend.
- No knowledge of the IdP's session cookie, user table, or login flow — all of that is opaque
