...
including one it doesn't own, to trick the browser into treating some other site as an IdP. The
well-known file — fetched from the IdP's own origin, without credentials, and which must not
redirect — is the IdP explicitly vouching "yes, this config URL is mine."
...
**Why it exists:** a single place listing every other endpoint, plus branding shown in the account
chooser (background color, icon).

**Implementation:** `internal/idp/handlers_config.go`, `handleConfig`. Fetched without credentials;
we defensively check `Sec-Fetch-Dest: webidentity` (the header Chrome/Edge attach to every FedCM
request) even though nothing else could realistically be asking for this file. Response:
...
knows what they're agreeing to share data with. Fetched uncredentialed, but *with* knowledge of
which RP (`client_id`) is asking — which is exactly why §2.1's stricter well-known requirement
exists, to stop this RP-awareness from being (ab)used to vary account data per RP undetectably.

**Implementation:** `internal/idp/handlers_client_metadata.go`. Needs
`Access-Control-Allow-Origin` (no credentials involved, so no `Allow-Credentials` needed) because,
unlike the other FedCM endpoints, this fetch is subject to normal CORS — see §2.7.
...
session for." Deliberately **not** RP-aware (see §1) — the browser cross-references the response
against the requesting RP's `client_id` itself.

**Implementation:** `internal/idp/handlers_accounts.go`, `handleAccounts`.

- No session cookie → **401**. This matters: it's what lets the Login Status API's mismatch
  handling work (§2.6) — if the IdP claimed "logged-in" but accounts says otherwise, the browser
  corrects itself.
- With a session → returns the account, including `approved_clients`: every `client_id` this user
  ...
  screen — the browser checks whether the requesting RP is already in that list.
...
2. Re-check the session cookie (still must be signed in).
3. Read `account_id`, `client_id`, `nonce` from the POST form and cross-check `account_id` against
   the signed-in user — a client can't assert a token for an account it didn't actually pick.
4. Mint a JWT via `internal/jwtutil/token.go` (`Mint`): `iss` = IdP origin, `sub` = account id,
   `aud` = `client_id`, plus profile claims and the `nonce` (carried in the JWT `jti` field so a
   replay-detection scheme could use it — this demo doesn't check it against anything stored,
   see the limitations note in the top-level README).
...
**Why it exists:** the browser needs to know, ahead of even trying the accounts endpoint, whether
...
- `handleLoginSubmit` (`POST /login`) — on success: creates an in-memory session
  (`internal/idp/session.go`), sets the `idp_session` cookie, and sets
  `Set-Login: logged-in`. The response body is a tiny page whose script calls
  `IdentityProvider.close()` — this is what tells the browser "the popup's done its job, close it
  and retry the accounts fetch automatically." Because `window.IdentityProvider` actually exists on
  *every* page (not just inside that special popup), `close()` silently no-ops if this page was
  loaded as a normal top-level navigation instead — so the script also arms a fallback
  `setTimeout` redirect to `/`, in case `close()` doesn't actually tear the page down.
...
itself from a user's IdP account (e.g. an "unlink your Google account" button), without needing a
...
lists every RP a user has granted, with its own "Revoke" button — this is plain form-based
self-service revocation, not part of the FedCM API at all, just a normal use of the same
`grants` table from the IdP's own UI.

## 2.8 CORS — the part that's easy to get wrong

Despite being invoked internally by the browser rather than by a page's `fetch()`, the
credentialed FedCM endpoints (`accounts`, `assertion`, `disconnect`) and the client metadata
endpoint are still subject to real CORS checks in current Chrome/Edge. Every one of them —
**including error responses** — needs:
...
See `setCORSHeaders` in `internal/idp/server.go`, called at the very top of each handler (before
any early-return error path) — a response missing these headers surfaces to the RP page as a
generic `ERR_FAILED` / "Server did not send the correct CORS headers" network error that gives no
hint about what actually went wrong server-side, which is what makes this easy to miss while
debugging.
